var Proof = /** @class */ (function () {
    function Proof(template) {
        this.template = template;
        this.studentAnswers = [];
    }
    Proof.prototype.answerStudent = function (answer) {
    ...
    Proof.prototype.hits = function () {
        var correctAnswers = 0; // inicia o contador    
        for (var i = 0; i < this.template.answers.length; i++) { // compara a resposta do template com a do estudante
    ...
    Proof.prototype.grade = function () {
        var score = 0;
        for (var i = 0; i < this.template.answers.length; i++) {
            if (this.template.answers[i] === this.studentAnswers[i]) {
                score += (i < 10) ? 0.5 : 1;
            }
        }
    ...
    Proof.prototype.greater = function (otherExam) {
        var thisGrade = this.grade();
        var otherGrade = otherExam.grade();
...
// Examplo de uso
var template = { answers: ['A', 'B', 'C', 'D', 'E', 'A', 'B', 'C', 'D', 'E', 'A', 'B', 'C', 'D', 'E'] };
var studentExam1 = new Proof(template);
var studentExam2 = new Proof(template);
